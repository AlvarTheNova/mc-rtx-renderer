...
- [x] **Descriptor-set strategy ✓ scoped:** spirv-cross reflection wired (vk_reflect.h/cpp). Validated in-MC against 3 real Mojang pipelines. **Key finding:** Mojang's shaders all use **single descriptor set (set=0)** with small binding counts (3-7), and same name in vs+fs maps to same binding (stage-shared). E.g. gui_textured: ubo 'DynamicTransforms' at binding=0 (vs+fs), ubo 'Projection' at binding=1 (vs), sampler 'Sampler0' at binding=1 (fs). Layout creation can be fully automatic from reflection output.
