...
void main() {
    // Sample the atlas. Don't discard on alpha — earlier "only lava visible"
    // bug came from atlas pixels having alpha=0 (most sprites). Force opaque
    // so we can see if RGB content is right.
